...
app.conf.update(
    ...
    # 队列路由
    task_routes={
        "app.tasks.generation.generate_image": {"queue": "image"},
        "app.tasks.generation.generate_video": {"queue": "video"},
        "app.tasks.safety.moderate_text": {"queue": "safety"},
        "app.tasks.safety.moderate_image": {"queue": "safety"},
    },
)

# 自动发现任务（app.tasks 包下的 task 模块）
app.autodiscover_tasks(["app"], "tasks")

# 显式导入确保任务注册（autodiscover 在某些启动方式下不会立即触发）
try:
    from app.tasks import generation, outbox, safety  # noqa: F401, E402
except ImportError:
    pass
